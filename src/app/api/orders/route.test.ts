...
const mockMenuItems = [{ id: MENU_ITEM_ID, price: 12.99, available: true }]
const mockOrder = { id: 'order-1', customer_name: 'Alice', customer_email: null, total: 12.99, status: 'pending', notion_page_id: null, created_at: '' }
...
function makeRequest(body: unknown) {
  // NextRequest body streaming is unavailable in the test environment.
  // The route only calls req.json(), so a minimal cast is sufficient.
...
describe('POST /api/orders', () => {
  ...
  it('returns 422 for unavailable menu item', async () => {
    const mock = makeSupabaseMock()
    // Override menu_items to return unavailable item
          ...
          return {
            select: vi.fn().mockReturnValue({
              in: vi.fn().mockResolvedValue({
                data: [{ id: MENU_ITEM_ID, price: 12.99, available: false }],
                error: null,
              }),
            }),
          }
